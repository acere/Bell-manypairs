...
/*
Calculates the majority voting for each chunk of length N
...
int majority(unsigned int *vec, int N){
    int counter = 0;
    int answ;
    for (int k = 0; k < N; k++){
        if(counter==0){
...
int maj_corr(unsigned int *vec, int N){
    unsigned int a_vec[N];
    unsigned int b_vec[N];
    int answ;
    ...
    if(majority(a_vec, N)==majority(b_vec, N)){
        answ = 1;
    }
    else {
        answ = -1;
